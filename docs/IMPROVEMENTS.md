...
4. **Corrigir `moduleResolution`** — está como `"node"`, depreciado para ESM. Com `"type": "module"` e `"module": "ESNext"` no `tsconfig.json`, o correto seria `"Bundler"` ou `"NodeNext"`.
5. **Remover `nodemailer`** das dependências, se não há planos de uso — ou implementar o `EmailAlertNotifier` já planejado (ver item 10).
...
10. **Notificação de sessão expirada no fluxo `findNewSuppliers`** — o `PuppeteerCommentPoster` já detecta quando está deslogado (check `a[href*="/login"]`) e lança erro com mensagem clara. Falta reagir a esse erro enviando um alerta (email ou webhook). Estrutura sugerida:
